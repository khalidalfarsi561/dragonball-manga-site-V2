			...
			<ul class="flex flex-col gap-4">
				{#each data.history.items as record (record.id)}
					<li
						...
						class="relative rounded-xl bg-gray-800 p-4 shadow-md ring-1 ring-white/5 transition-all duration-300 hover:shadow-lg {selected.has(
							record.id
						)
						...
						<div class="flex items-start gap-4">
							...
							<div class="flex-grow">
								...
								<div class="mt-4 flex items-center gap-3 md:hidden">
									...
									<form method="POST" action="?/deleteRecord" class="absolute bottom-3 left-3 m-0">
										<input type="hidden" name="id" value={record.id} />

										<button
											type="submit"
											class="rounded-full p-2 text-red-400 transition hover:bg-red-500/20 hover:text-red-300"
											aria-label="حذف"
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
