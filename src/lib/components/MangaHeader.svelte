				...
				<div class="relative inline-block text-left">
					<div>
						...
						>
							إجراءات
							<svg
								class={`h-5 w-5 text-gray-200 transition-transform duration-300 ${
					...
					{#if isActionsMenuOpen}
						<div
							...
							onkeydown={(e) => {
								if (e.key === 'Escape' || e.key === 'Enter' || e.key === ' ') closeMenu();
							}}
							tabindex="0"
							class="ring-opacity-5 absolute top-0 right-full z-20 mr-2 w-60 origin-right rounded-xl bg-gray-800/80 shadow-2xl ring-1 ring-black backdrop-blur-sm focus:outline-none"
							role="menu"
									...
									>
										...
										{:else}
											...
											<svg
												...
												stroke="currentColor"
												class={`h-5 w-5 ${
													isFavorited ? 'text-red-500' : 'text-gray-400 group-hover:text-red-500'
